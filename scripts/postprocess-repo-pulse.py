#!/usr/bin/env python3
from __future__ import annotations

from pathlib import Path
import re
import sys


def fail(message: str) -> None:
    raise SystemExit(f"repo pulse postprocess: {message}")


def fmt(value: float) -> str:
    return f"{value:.3f}".rstrip("0").rstrip(".")


def replace_once(text: str, old: str, new: str, label: str) -> str:
    count = text.count(old)
    if count != 1:
        fail(f"expected one {label} target, found {count}")
    return text.replace(old, new, 1)


def horizontal_extent(d: str) -> tuple[float, float]:
    start = re.match(r"M(-?[0-9]+(?:\.[0-9]+)?)\s+-?[0-9]+(?:\.[0-9]+)?", d)
    if not start:
        fail("could not parse ECG start point")
    x0 = float(start.group(1))
    x = x0
    for value in re.findall(r"\bh(-?[0-9]+(?:\.[0-9]+)?)", d):
        x += float(value)
    for dx, _dy in re.findall(
        r"\bl(-?[0-9]+(?:\.[0-9]+)?)\s+(-?[0-9]+(?:\.[0-9]+)?)", d
    ):
        x += float(dx)
    for _cpx, _cpy, dx, _dy in re.findall(
        r"\bq(-?[0-9]+(?:\.[0-9]+)?)\s+(-?[0-9]+(?:\.[0-9]+)?)\s+"
        r"(-?[0-9]+(?:\.[0-9]+)?)\s+(-?[0-9]+(?:\.[0-9]+)?)",
        d,
    ):
        x += float(dx)
    return x0, x


path = Path(sys.argv[1] if len(sys.argv) > 1 else "/tmp/pulse.svg")
svg = path.read_text()

if 'viewBox="0 0 830 260"' not in svg:
    fail("expected 830x260 Wide+ SVG")

# Repository cards keep their repo name at the top-left, but otherwise share the
# Profile composition: one full-width ECG plus a compact four-module bottom strip.
# Repo semantics remain HEART RATE / OPEN PRS / OPEN ISSUES / STREAK · TYPE.

# Compact heartbeat accent in the first bottom module.
heart_re = re.compile(
    r'<text class="gp-heart" x="26" y="98"(?P<attrs>[\s\S]*?)font-size="18"(?P<tail>[\s\S]*?)>♥</text>'
)
if not heart_re.search(svg):
    fail("expected one renderer heart glyph")
svg, heart_count = heart_re.subn(
    lambda m: (
        '<text class="gp-heart" x="22" y="222"'
        + m.group('attrs')
        + 'font-size="11.5"'
        + m.group('tail')
        + '>♥</text>'
    ),
    svg,
    count=1,
)
if heart_count != 1:
    fail(f"expected one renderer heart glyph, found {heart_count}")

# Heart speed follows the displayed BPM with the same mapping as Profile.
bpm_match = re.search(r'<title[^>]*>.*?—\s*([0-9]+)\s*bpm', svg)
if not bpm_match:
    fail("BPM not found in SVG title")
bpm = max(1.0, float(bpm_match.group(1)))
heart_duration = max(0.62, min(1.65, 120.0 / bpm))
svg, heart_css_count = re.subn(
    r'(\.gp-heart\{animation:gp-thump )[0-9.]+(s ease-in-out infinite;transform-origin:center;transform-box:fill-box\})',
    lambda m: m.group(1) + fmt(heart_duration) + m.group(2),
    svg,
    count=1,
)
if heart_css_count != 1:
    fail(f"expected one gp-heart CSS rule, found {heart_css_count}")
svg, thump_css_count = re.subn(
    r'@keyframes gp-thump\{0%,100%\{transform:scale\(1\)\}15%\{transform:scale\(1\.32\)\}30%\{transform:scale\(1\)\}\}',
    '@keyframes gp-thump{0%,100%{transform:scale(1)}20%{transform:scale(1.12)}40%{transform:scale(1)}}',
    svg,
    count=1,
)
if thump_css_count != 1:
    fail(f"expected one gp-thump keyframe, found {thump_css_count}")

# Four equal bottom modules. Only HEART RATE label shifts right to make room for
# the small heart; all values keep their module left axes.
strip_positions = [
    ('x="26" y="72"', 'x="36" y="222"', "heart-rate label"),
    ('x="48" y="98"', 'x="22" y="246"', "heart-rate value"),
    ('x="26" y="120"', 'x="219" y="222"', "open-prs label"),
    ('x="26" y="146"', 'x="219" y="246"', "open-prs value"),
    ('x="26" y="168"', 'x="415" y="222"', "open-issues label"),
    ('x="26" y="194"', 'x="415" y="246"', "open-issues value"),
    ('x="26" y="216"', 'x="612" y="222"', "streak label"),
    ('x="26" y="242"', 'x="612" y="246"', "streak value"),
]
for old, new, label in strip_positions:
    svg = replace_once(svg, old, new, label)

# Full-width ECG field; the old rail divider becomes the bottom-strip separator.
svg = replace_once(
    svg,
    'x="200" y="48" width="614" height="160"',
    'x="22" y="42" width="786" height="148"',
    "ECG grid",
)
svg = replace_once(
    svg,
    'x1="190" y1="48" x2="190" y2="244"',
    'x1="22" y1="207" x2="808" y2="207"',
    "bottom strip divider",
)
svg = replace_once(
    svg,
    'x1="210" y1="128" x2="804" y2="128"',
    'x1="22" y1="126" x2="808" y2="126"',
    "level baseline",
)

# Match the Profile trace hierarchy: visible resting trace, restrained glow,
# one travelling emphasis segment.
svg = replace_once(
    svg,
    '<feGaussianBlur stdDeviation="2.6" result="b"/>',
    '<feGaussianBlur stdDeviation="1.7" result="b"/>',
    "ECG glow blur",
)
svg, base_count = re.subn(
    r'(fill="none" stroke="[^"]+" stroke-width=")1\.7(" opacity=")0\.20(")',
    r'\g<1>1.8\g<2>0.40\g<3>',
    svg,
    count=1,
)
if base_count != 1:
    fail(f"expected one resting ECG stroke, found {base_count}")
svg, trail_count = re.subn(
    r'(class="gp-trail"[\s\S]*?stroke="[^"]+" stroke-width=")5\.2(" stroke-linecap="round"\s+stroke-dasharray=")250 750(" opacity=")0\.11(")',
    r'\g<1>4.0\g<2>170 830\g<3>0.07\g<4>',
    svg,
    count=1,
)
if trail_count != 1:
    fail(f"expected one ECG trail style target, found {trail_count}")
svg, sweep_count = re.subn(
    r'(class="gp-sweep"[\s\S]*?stroke="[^"]+" stroke-width=")2\.6(" stroke-linecap="round"\s+stroke-dasharray=")92 908(")',
    r'\g<1>2.6\g<2>170 830\g<3>',
    svg,
    count=1,
)
if sweep_count != 1:
    fail(f"expected one ECG sweep style target, found {sweep_count}")

# Preserve inactive gaps from repository history while replacing each active beat
# with the same broader P-QRS-T morphology used by Profile.
source_beat_re = re.compile(
    r'q3 -([0-9]+(?:\.[0-9]+)?) 6 0 '
    r'l2 3 l4 -([0-9]+(?:\.[0-9]+)?) l4 [0-9]+(?:\.[0-9]+)? l2 -11 '
    r'q3 -([0-9]+(?:\.[0-9]+)?) 6 0'
)
trace_re = re.compile(
    r'(<path(?: class="gp-(?:trail|sweep)")? d=")(M210 128[^"]+)("[^>]*/>)'
)
TARGET_X0 = 22.0
TARGET_X1 = 808.0
BASELINE_SHIFT = -2.0
trace_count = 0
amplitude_sets: list[list[float]] = []


def reshape_trace(match: re.Match[str]) -> str:
    global trace_count
    source_d = match.group(2)
    amplitudes = [float(m.group(2)) for m in source_beat_re.finditer(source_d)]
    amplitude_sets.append(amplitudes)
    trace_count += 1

    if not amplitudes:
        return match.group(1) + f"M{fmt(TARGET_X0)} 126 H{fmt(TARGET_X1)}" + match.group(3)

    peak = max(amplitudes) or 1.0
    dominant_index = amplitudes.index(max(amplitudes))
    beat_index = 0

    def replace_beat(beat: re.Match[str]) -> str:
        nonlocal beat_index
        amplitude = float(beat.group(2))
        normalized = max(0.0, min(1.0, amplitude / peak))
        if beat_index == dominant_index:
            rise = 72.0
        else:
            rise = min(46.0, 10.0 + 62.0 * (normalized ** 2.4))
        t_control = 12.0 + 5.0 * normalized
        beat_index += 1
        return (
            "q3 -5.5 6 0 h2 l2 4 "
            f"l12 -{fmt(rise + 4.0)} l14 {fmt(rise + 6.0)} "
            f"l3 -6 h3 q4 -{fmt(t_control)} 8 0"
        )

    d = source_beat_re.sub(replace_beat, source_d)
    raw_start, raw_end = horizontal_extent(d)
    raw_span = raw_end - raw_start
    if raw_span <= 0:
        fail(f"invalid ECG raw span: {raw_span}")
    scale_x = (TARGET_X1 - TARGET_X0) / raw_span
    translate_x = TARGET_X0 - scale_x * raw_start
    matrix = f"matrix({scale_x:.6f} 0 0 1 {translate_x:.3f} {fmt(BASELINE_SHIFT)})"
    tail = match.group(3)
    return (
        match.group(1)
        + d
        + f'" transform="{matrix}" vector-effect="non-scaling-stroke"'
        + tail[1:]
    )


svg = trace_re.sub(reshape_trace, svg)
if trace_count != 3:
    fail(f"expected base/trail/sweep ECG paths, found {trace_count}")
if not all(values == amplitude_sets[0] for values in amplitude_sets[1:]):
    fail("ECG layers disagree on source activity amplitudes")

# Anchor state chrome to the same outer right edge as Profile.
pill_re = re.compile(
    r'(<rect class="gp-pill-pulse" x=")([0-9.]+)(" y="18" width=")([0-9.]+)(" height="19")'
)
pill = pill_re.search(svg)
if not pill:
    fail("state pill not found")
pill_width = float(pill.group(4))
pill_x = 808.0 - pill_width
svg, count = pill_re.subn(
    lambda m: m.group(1) + fmt(pill_x) + m.group(3) + m.group(4) + m.group(5),
    svg,
    count=1,
)
if count != 1:
    fail(f"expected one state pill, found {count}")
state_text_re = re.compile(r'(<text x=")([0-9.]+)(" y="31" text-anchor="middle")')
svg, count = state_text_re.subn(
    lambda m: m.group(1) + fmt(pill_x + pill_width / 2.0) + m.group(3),
    svg,
    count=1,
)
if count != 1:
    fail(f"expected one state label, found {count}")
svg = replace_once(
    svg,
    'x="804" y="238" text-anchor="end"',
    'x="808" y="198" text-anchor="end"',
    "ECG lower-right status",
)

# Positive guards: no old rail geometry survives; exactly one compact heart does.
for forbidden in ('x="200" y="48" width="614"', 'x1="190" y1="48" x2="190" y2="244"'):
    if forbidden in svg:
        fail(f"old rail geometry survived: {forbidden}")
if svg.count('♥') != 1 or svg.count('class="gp-heart"') != 1:
    fail("expected exactly one compact heartbeat accent")
if 'font-size="11.5"' not in svg or 'x="22" y="222"' not in svg:
    fail("compact heartbeat accent geometry missing")

path.write_text(svg)
print(
    "Repository Pulse postprocess verified: full-width ECG + four-module bottom strip; "
    f"beats={len(amplitude_sets[0])}; bpm={bpm:g}; heart={fmt(heart_duration)}s"
)
